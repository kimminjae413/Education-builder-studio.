...
// 이 API는 Vertex AI Search 자동 인덱싱으로 대체됨
// 폴백용으로 유지
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Vertex AI Search가 자동으로 인덱싱하므로
    // 이 API는 단순히 인덱싱 완료된 자료를 표시하는 용도로 변경

    // 인덱싱 대기 중인 자료 조회
    const materials = await getMaterialsForVertexIndexing(100)

    if (!materials || materials.length === 0) {
      console.log('✅ 모든 자료가 Vertex AI에 인덱싱되었습니다')
      return NextResponse.json({
        message: 'All materials already indexed in Vertex AI',
        processed: 0,
      })
    }
    ...
    for (const material of materials) {
      try {
        // GCS에 파일이 업로드되면 Vertex AI가 자동 인덱싱
        // 여기서는 플래그만 업데이트
    ...
    return NextResponse.json({
      ...
      message: 'Vertex AI Search는 GCS 버킷과 자동 동기화됩니다',
    })
  } catch (error) {
